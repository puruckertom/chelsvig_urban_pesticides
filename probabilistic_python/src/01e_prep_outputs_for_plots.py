...
for o in outfalls:
    for o in outfalls:
        ...
        swmm_df = pd.read_table(swmm_out, header=None,
        ...
        runf.to_csv(outfall_path + r'\determ_runf.txt', index=False)

        # vvwm output time series file (vvwm .csv)
        vvwm_df = pd.read_table(vvwm_out, header=None, names=['depth', 'conc_h20', 'conc_benth', 'conc_peak'], sep=",",
                                skiprows=5)
        ...
        conc_h20.to_csv(outfall_path + r'\determ_conc_h20.txt', index=False)

        conc_benth = vvwm_df.loc[:, ['conc_benth', 'date']]
        conc_benth['conc_benth'] = conc_benth['conc_benth'].apply(lambda x: x * 1000000)  # ug/L
        conc_benth.to_csv(outfall_path + r'\determ_conc_benth.txt', index=False)
