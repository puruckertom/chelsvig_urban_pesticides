...
read.csv(file=paste0(calpip_dir,"placer_09-17.csv"), header=T) %>% 
  mutate(bif_kg_with_home = bif_kg*1.25) %>% 
  write.csv(file=paste0(calpip_dir,"placer_09-17_with_homeowner.csv"), row.names=F)
