...
# go
read.table(paste0(calpip_dir, "app_rate_output_for_swmm.txt"),
           header = F, col.names = c('date','time','apps_kgha'),
           stringsAsFactors = F) %>% select(-time) %>% 
  mutate(date = seq(from=as.Date("2009-01-01"), to = as.Date("2017-12-31"), by = "day")) %>% 
  mutate(precip_mm = (read.csv(file=paste0(dir_weather, "Precipitation_nldas_daily.csv"), 
                               header=TRUE, sep=",", nrows = 3287))$DailyTotal_kgm2) %>% 
  mutate(rainbuff = NA, apps_update_kgha = apps_kgha)%>% 
  mutate(rainbuff = as.numeric(
    filter(c(0,precip_mm), filter = rep(1/3,3), circular = T)[-1] != 0)) %>% 
  mutate(apps_update_kgha = 
           ifelse(rainbuff==1,0,apps_update_kgha))%>% 
  separate(date, sep="-", into = c("year", "month", "day"), remove = F) %>% 
  mutate(app_diff = apps_kgha - apps_update_kgha,
         apps_update2_kgha = NA) %>% 
  group_by(year, month) %>% 
  mutate(apps_update2_kgha = 
           apps_update_kgha + (1-rainbuff)*(
             sum(app_diff)/(sum(1-rainbuff))
           )) %>% ungroup() %>% 
  mutate(apps_update2_imp_kgha = apps_update2_kgha * 0.7362,
         time = '01:00',
...
# 
# # read in apps and precip files and merge them into one table with columns for rainbuff and update
# 
# 
# # read in apps and precip files
# daily_apps <- read.table(paste0(calpip_dir, "app_rate_output_for_swmm.txt", sep=''), header = F)#,skip = 3, header=F) #JMS 10/26/20
...
#     
#     # divide by number of days that get apps, to average
...
# 
# # read out
# write.table(out_file, paste0(calpip_dir, "app_rate_output_for_swmm_48rain.txt", sep=""), quote=F, sep="\t", row.names=F, col.names=F)
# 
# 
# # ----------------------------------------------------------------
# # the end
