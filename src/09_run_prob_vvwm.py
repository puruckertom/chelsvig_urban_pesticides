...
for o in outfalls:
    ...
    for i in range(1, nsims + 1):
        sim_dir = os.path.join(outfall_dir, "input_" + str(i))#outfall_dir + r'\input_' + str(i)

        # copy weather file into new file location
        old_wet_path = os.path.join(wet_path, "vvwm_wet.dvf")#wet_path + r'\vvwm_wet.dvf'
        print(old_wet_path)
        new_wet_path = os.path.join(sim_dir, "vvwm_wet.dvf")#sim_dir + r'\vvwm_wet.dvf'
        print(new_wet_path)
        ...
        new_exe_path = os.path.join(sim_dir, "VVWM.exe")#sim_dir + r'\VVWM.exe'
        ...
        new_path = os.path.join(sim_dir, "vvwmTransfer.txt")#sim_dir + r'\vvwmTransfer.txt'
